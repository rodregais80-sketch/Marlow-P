...
def _ensure_tables(db: DatabaseManager):
    """Creates compressed_memories, pinned_memories, and consolidation_log tables."""
    db.conn.execute("""
...
def _score_relevance(text: str, persona_name: str) -> int:
    """Returns a relevance score for text against a persona's domain keywords."""
    if not text:
        return 0
    ...
    for persona_name, domain in PERSONA_DOMAINS.items():
        ...
        if persona_name in ("ALDRIC", "MORRO") and decisions:
            for d in decisions[:3]:
                desc    = (d.get("decision") or d.get("description") or d.get("decision_text") or "")[:100]
                outcome = d.get("outcome_rating") or d.get("quality_rating") or d.get("outcome_score")
                if desc:
...
def _compress_month_groq(
    ...
    Pulls weekly summaries for that month and synthesizes into a narrative.
    Returns a dict: {persona_name: compressed_string}
    """
    ...
    for persona_name, domain in PERSONA_DOMAINS.items():
        ...
        if not weekly_in_range:
            ...
            raw_text = ""
            if metrics and domain["metric_fields"]:
                field = domain["metric_fields"][0]
                vals  = [m.get(field) for m in metrics if m.get(field) is not None]
                if vals:
...
def maybe_consolidate_memory(db: DatabaseManager) -> list:
    ...
    Call once at startup in marlow.py after db is initialized.
    """
    _ensure_tables(db)
    actions = []
    now     = datetime.now()
    ...
    should_run_weekly = (last_weekly is None or (now - last_weekly).days >= 7)

    if should_run_weekly:
        cutoff = now - timedelta(days=30)
        try:
            oldest_row = db.conn.execute("SELECT MIN(timestamp) FROM daily_metrics").fetchone()
        ...
        if oldest_ts:
            ...
            week_cursor -= timedelta(days=week_cursor.weekday())   # align to Monday
            weeks_compressed = 0
    ...
    should_run_monthly = (last_monthly is None or (now - last_monthly).days >= 30)
    ...
    if should_run_monthly:
        ...
        if months_compressed > 0:
            actions.append(
                f"Monthly compression: {months_compressed} month(s) processed "
                f"({months_compressed * 4} Groq calls used)"
            )
    ...
    should_run_annual = (last_annual is None or (now - last_annual).days >= 365)
    ...
    if should_run_annual:
        ...
        if years_compressed > 0:
            actions.append(
                f"Annual compression: {years_compressed} year(s) archived "
                f"({years_compressed * 4} Groq calls used)"
            )
...
def get_tiered_context_for_persona(db: DatabaseManager, persona_name: str) -> str:
    ...
    What gets returned:
      ...
      - Raw logs: last 30 days (handled by memory.py — not included here)
      - Pinned memories: always included regardless of age

    Injected AFTER the raw memory block (which covers last 30 days).
    ...
    if pinned:
        pin_lines = [
            f"  * {p['content']} [pinned {p['pinned_at'][:10]}]"
            for p in pinned
        ]
    ...
    if annual_in_range:
        parts.append("--- ANNUAL ARCS ---\n" + "\n\n".join(r[2] for r in annual_in_range))
...
def get_pinned_memories(db: DatabaseManager, persona_name: str = None) -> list:
    """
    Returns pinned memories.
    If persona_name is given, filters to that persona only.
    Returns list of dicts: {id, persona_name, content, pinned_at, pinned_by}
    """
    _ensure_tables(db)
    if persona_name:
        rows = db.conn.execute(
            """SELECT id, persona_name, content, pinned_at, pinned_by
               FROM pinned_memories WHERE persona_name = ?
        ...
        rows = db.conn.execute(
            """SELECT id, persona_name, content, pinned_at, pinned_by
               FROM pinned_memories ORDER BY persona_name, pinned_at DESC"""
        ).fetchall()
    return [dict(r) for r in rows]
...
def list_all_pinned(db: DatabaseManager) -> str:
    ...
    for p in pins:
        if p["persona_name"] != current_persona:
            ...
            lines.append(f"\n  -- {current_persona} --")
        lines.append(
            f"  [{p['id']}] {p['content'][:120]} ({p['pinned_by']}, {p['pinned_at'][:10]})"
        )
