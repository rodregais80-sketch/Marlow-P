...
Design intent:
  - build_shared_context() is called ONCE per council run by council_engine.py
  - The resulting dict is passed to build_context_for_persona() for each persona
  - Each persona receives only the context fields relevant to their domain
  - Character limits guard against silent context window overflow

Improvements addressed:
  - Tiered context delivery per persona domain (reduces wasted tokens)
  - Context built once per session (not rebuilt on every parallel call)
  - Character limit guards protecting context window integrity
  - Recent sync data injected for all relevant personas (Swap 15 alignment)
"""
...
MAX_SYNC_CHARS     = 600   # Recent sync injection cap
...
# Which personas receive behavioral trend reports in their context
PERSONA_NEEDS_TRENDS = {
    "ALDRIC": True,
    "SEREN":  True,
    "MORRO":  False,   # MORRO reacts to impulse, not data analysis
    "ORYN":   True,
}
...
# Which personas receive recent sync injection
PERSONA_NEEDS_RECENT_SYNCS = {
    "ALDRIC": True,
    "SEREN":  True,
    "MORRO":  False,
    "ORYN":   True,
...
def build_shared_context(
    ...
    DB calls across parallel persona execution.

    Injects recent sync logs (last 3 entries) for all relevant personas.
    This is the Swap 15 alignment — PersonaChat context now shows recent syncs.
    ...
    if profile:
        profile_block = (
            ...
            f"Biggest Challenge: {profile.get('biggest_challenge') or ''}\n"
            f"Preferred Support Style: {profile.get('support_style') or ''}\n"
            f"Additional Context: {profile.get('additional_context') or ''}"
        )
    ...
    # ── Recent sync injection (Swap 15 alignment) ──────────────────────────
    # Fetch last 3 sync logs for context injection into relevant personas.
    # Gives personas real-time grounding without requiring PersonaChat rebuild.
    recent_syncs_block = ""
    try:
        ...
        if recent_logs:
            ...
            for log in recent_logs:
                try:
                    ts      = str(log["timestamp"])[:16] if log.get("timestamp") else "unknown"
                    stype   = str(log.get("sync_type", "sync")).upper()
                    content = str(log.get("content", ""))[:300]
                    sync_lines.append(f"[{ts} — {stype}]\n{content}")
                except Exception:
                    continue
            if sync_lines:
                recent_syncs_block = "\n\n".join(sync_lines)
    ...
    return {
        ...
        "recent_syncs_block":  recent_syncs_block[:MAX_SYNC_CHARS],
    }
...
def build_context_for_persona(persona_name: str, shared: dict) -> str:
    ...
    MORRO receives minimal context (background only, no trends, no memory, no syncs).
    ALDRIC/SEREN/ORYN receive full tiered context within character limits.
    ...
    # ── Memory and cross-session context — ALDRIC, SEREN, ORYN only ──────
    if PERSONA_NEEDS_MEMORY.get(persona_name, True) and shared.get("memory_block"):
        parts.append(f"--- MEMORY & CROSS-SESSION CONTEXT ---\n{shared['memory_block']}")

    # ── Recent syncs — ALDRIC, SEREN, ORYN only (Swap 15 alignment) ──────
    if PERSONA_NEEDS_RECENT_SYNCS.get(persona_name, True) and shared.get("recent_syncs_block"):
        parts.append(f"--- RECENT SYNCS ---\n{shared['recent_syncs_block']}")
