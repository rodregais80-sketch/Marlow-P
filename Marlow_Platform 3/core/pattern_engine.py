...
# Minimum data points before pattern analysis is meaningful
MIN_ENTRIES_FOR_PATTERNS  = 10
MIN_ENTRIES_FOR_TEMPORAL  = 14
MIN_ENTRIES_FOR_SEQUENCE  = 20

# Emotional dimensions extracted from language
EMOTION_LEXICON = {
...
# Positive emotional dimensions (for happiness signature mapping)
POSITIVE_EMOTIONS = {"joy", "excitement", "pride", "peace", "drive", "connection", "hunger"}

# Stop words for linguistic analysis
_STOP_WORDS = {
...
class PatternEngine:
    ...
    This is what makes it personal rather than generic.
    """
    ...
    def analyze_emotional_signatures(self) -> dict:
        ...
        Returns a rich dict with emotional baseline, range, and dominant dimensions.
        """
        logs    = self._load_logs(limit=90)
        journals = self._load_journals(limit=60)

        all_texts = []
        timestamped_emotions = []   # [(timestamp, emotion_scores)]
        ...
        return {
            ...
            "emotional_valence":   emotional_valence,   # 0=entirely negative, 1=entirely positive
            "emotional_range":     sorted_emotions,     # full ranked list
        }
    ...
    def map_flourishing_conditions(self) -> dict:
        ...
        This is the personalized map of what makes THIS person genuinely flourish.
        Not generic wellness advice — specific conditions derived from their data.

        Returns conditions that predict: high mood, high energy, high drive.
        """
        ...
        # Find high-state days: energy >= 7 AND mood >= 7
        high_state_entries  = []
        low_state_entries   = []
        ...
        if not high_state_entries:
            return {"status": "no_high_state_data", "note": "No entries found with energy AND mood >= 7"}

        # Analyze conditions in high-state entries
        ...
        best_hours   = [h for h, c in hour_counts.most_common(3)]
        best_sync    = [s for s, c in sync_type_counts.most_common(2)]

        # Language in high-state log entries on same days
        ...
        low_word_freq  = Counter(low_words).most_common(20)

        # Words unique to high states (not in low)
        ...
        return {
            ...
            "low_state_language":   low_word_freq[:10],
    ...
    def map_danger_signature(self) -> dict:
        ...
        This is predictive — the goal is to catch the pattern while there is still
        time to intervene.
        """
        ...
        # Find crash events: energy drops 3+ points across any 3-entry window
        for i in range(2, len(energy_series)):
            ts0, e0 = energy_series[i-2][0], energy_series[i-2][1]
            ...
            if e0 - e2 >= 3:
                ...
                # Capture the 5 entries before the peak
                pre_start = max(0, i - 7)
        ...
        # Aggregate pre-crash conditions
        ...
        # How similar is current state to pre-crash pattern?
        match_signals = 0
        ...
        return {
            ...
            "pre_crash_signature": {
                ...
                "note":         "These are average conditions in the 5 entries before a crash."
            },
            "crash_events":             crash_events[-5:],   # last 5 crash events
            "current_signature_match":  match_pct,           # % match to pre-crash pattern
    ...
    def analyze_temporal_patterns(self) -> dict:
        ...
        # Sync-time averages (morning vs midday vs evening)
        sync_report = {}
        ...
        # Find peak hour (hour of day with highest avg energy)
        ...
        return {
            ...
            "note":         "All times are approximate based on sync timestamps."
        }
    ...
    def analyze_linguistic_patterns(self) -> dict:
        ...
        Finds:
        - Most frequently used words across all syncs (their psychological vocabulary)
        - Words that appear only when states are high (flourishing language)
        - Words that appear only when states are low (distress markers)
        - Recurring themes that appear across multiple weeks
        - Unique words — things they mention repeatedly that don't show up in stop lists
          (these are usually the most revealing)
        """
        metrics = self._load_metrics(limit=90)
        logs    = self._load_logs(limit=90)
        journals = self._load_journals(limit=60)
        ...
        # Words unique to each state
        low_set    = set(w for w, c in low_freq.most_common(100))
        high_set   = set(w for w, c in high_freq.most_common(100))
        ...
        return {
            ...
            "flourishing_language": unique_high,   # appear in high states, not low
            "distress_markers":     unique_low,    # appear in low states, not high
    ...
    def detect_sequences(self) -> dict:
        ...
        Examples the system looks for:
        - 3+ consecutive high energy days → crash follows
        - Low sleep 2 days in a row → fog spikes
        - High impulse 3 days → reckless decision event (impulse spike in logs)
        - Productive morning sync → evening drop
        """
        metrics = self._load_metrics(limit=120)
        ...
        if low_sleep_runs > 0:
            fog_after_sleep_pct = round(low_sleep_fog_pairs / low_sleep_runs * 100) if low_sleep_runs > 0 else 0
            sequences.append({
        ...
        # Pattern 4: Post-high-energy fog lag (energy spike followed by fog increase)
        energy_fog_lag = 0
        energy_spikes  = 0
        for i in range(n - 2):
            if energy_vals[i] >= 8:
        ...
        # Most impactful sequence (highest occurrences with predictive=True)
    ...
    def compute_correlation_matrix(self) -> dict:
        ...
        This reveals the hidden structure of how this operator's variables
        relate to each other — which findings are unique to them.
        """
        metrics = self._load_metrics(limit=90)
        ...
        # Fill None values with column mean
        for f in fields:
        ...
        # Extract notable correlations (abs > 0.4)
        notable = []
        seen    = set()
        for f1 in fields:
            for f2 in fields:
    ...
    def analyze_decision_quality(self) -> dict:
        """
        Examines logged decisions (from decision_log table) and their
        retrospective quality ratings.

        Finds: what conditions (energy, sleep, impulse, substances) were the operator
        in when they made their BEST decisions vs their WORST decisions.

        This is the anti-impulsivity map — shows the operator exactly when
        not to make major decisions.
        """
        try:
            cursor = self.db.conn.cursor()
            cursor.execute("""
                SELECT * FROM decision_log WHERE outcome_score IS NOT NULL
        ...
        for d in decisions:
            rating = _safe_val(d, "outcome_score", 5) or 5
            conds  = {
                "energy":   _safe_val(d, "state_energy"),
                "sleep":    _safe_val(d, "state_sleep"),
                "impulse":  _safe_val(d, "state_impulse"),
                "fog":      _safe_val(d, "state_fog"),
            }
        ...
        return {
            ...
            "insight": (
                f"Good decisions made at avg energy {comparison.get('energy',{}).get('good_decisions','?')}, "
    ...
    def score_goal_momentum(self) -> dict:
        ...
        A goal that appears 4 times in 7 days is moving.
        A goal that hasn't appeared in 14 days is stalling.
        """
        goals = self.db.get_active_goals()
        if not goals:
            return {"status": "no_active_goals"}

        # Pull recent text content
        ...
        for goal in goals:
            ...
            # Count mentions in last 7 days vs 7-14 days
            last_7_count  = 0
            last_14_count = 0
            ...
            # Momentum score: recent activity vs prior period
    ...
    def synthesize_master_insights(self) -> dict:
        ...
        This is what gets injected into council context and what the AI
        uses to generate pattern-aware responses.

        Expensive call — cache the result and regenerate weekly.
        """
        results = {}

        try:
            results["emotional_signatures"] = self.analyze_emotional_signatures()
        except Exception as e:
            results["emotional_signatures"] = {"status": "error", "msg": str(e)}

        try:
            results["flourishing_conditions"] = self.map_flourishing_conditions()
        except Exception as e:
            results["flourishing_conditions"] = {"status": "error", "msg": str(e)}

        try:
            results["danger_signature"] = self.map_danger_signature()
        except Exception as e:
            results["danger_signature"] = {"status": "error", "msg": str(e)}

        try:
            results["temporal"] = self.analyze_temporal_patterns()
        except Exception as e:
            results["temporal"] = {"status": "error", "msg": str(e)}

        try:
            results["linguistic"] = self.analyze_linguistic_patterns()
        except Exception as e:
            results["linguistic"] = {"status": "error", "msg": str(e)}

        try:
            results["sequences"] = self.detect_sequences()
        except Exception as e:
            results["sequences"] = {"status": "error", "msg": str(e)}

        try:
            results["correlations"] = self.compute_correlation_matrix()
        except Exception as e:
            results["correlations"] = {"status": "error", "msg": str(e)}

        try:
            results["decision_quality"] = self.analyze_decision_quality()
        except Exception as e:
            results["decision_quality"] = {"status": "error", "msg": str(e)}

        try:
            results["goal_momentum"] = self.score_goal_momentum()
        except Exception as e:
            results["goal_momentum"] = {"status": "error", "msg": str(e)}
    ...
    def format_insights_for_context(self, insights: dict = None) -> str:
        ...
        Called by context_builder.py to add pattern intelligence to every council session.
        """
        if insights is None:
        ...
        if gm.get("status") == "complete":
            ...
            if active:
                top_active = active[0]["goal"]
                lines.append(f"Highest momentum goal: {top_active}")

        # Notable correlations
        corr = insights.get("correlations", {})
        if corr.get("status") == "complete":
            notable = corr.get("notable", [])[:2]
            for n in notable:
...
# ─────────────────────────────────────────────────────────────────────────────
# STANDALONE UTILITY: Save/load cached pattern analysis to DB
# ─────────────────────────────────────────────────────────────────────────────
...
def load_pattern_cache(db) -> dict:
    ...
    try:
        ...
        if (datetime.now(timezone.utc) - gen_at.replace(tzinfo=timezone.utc)).total_seconds() > 86400:
            return {}
...
def build_pattern_context(pattern_data: dict, max_chars: int = 2000) -> str:
    ...
    if not pattern_data:
        return "Pattern Intelligence: Insufficient data. Continue logging to activate."
    try:
        result = PatternEngine(None).format_insights_for_context(pattern_data)
