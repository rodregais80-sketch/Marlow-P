...
# ── Session state — holds session-static data, fetched once ──────────────────
try:
...
# ── Background runner ─────────────────────────────────────────────────────────
try:
    from core.background_runner import start_background_runner, stop_background_runner
    _BACKGROUND_RUNNER_AVAILABLE = True
except ImportError:
    _BACKGROUND_RUNNER_AVAILABLE = False
    def start_background_runner(db, logger_fn=None): return False
    def stop_background_runner(): pass

# ── Persona learning engine ───────────────────────────────────────────────────
try:
    from core.persona_learning_engine import (
        trigger_learning_from_rating,
        format_learning_report
    )
    _LEARNING_ENGINE_AVAILABLE = True
except ImportError:
    _LEARNING_ENGINE_AVAILABLE = False
    def trigger_learning_from_rating(db, decision_id, groq_chat_fn): pass
    def format_learning_report(db): return ""
...
# Session state — initialized in main() after first-time intake check
# Holds profile, history, goals, monthly pattern, momentum, tiered histories
# Passed to run_council and other heavy functions to avoid repeated DB reads
SESSION = SessionState(db) if _SESSION_STATE_AVAILABLE else None
...
def run_vent_mode():
    ...
    if SESSION:
        SESSION.invalidate_after_journal()
    print()
    print("  Classifying intent...")
    classification = classify_intent(user_input)
    ...
    while True:
        ...
        if not goals:
            ...
            for g in goals:
                ...
                g_dict      = dict(g)
                print(f"  [{status_icon}] #{g_dict['id']} — {g_dict['title']}  [{g_dict['status'].upper()}]{stall}")
                if score_str:
                    print(f"       {score_str}")
                if g_dict.get("description"):
                    print(f"       {g_dict['description']}")
                if g_dict.get("progress_note"):
                    print(f"       Progress: {g_dict['progress_note']}")
                if g_dict.get("target_date"):
                    print(f"       Target: {g_dict['target_date']}")

                # Show plan progress inline if a plan exists
                if _STRATEGY_PLANNER_AVAILABLE:
                    plan = get_active_plan(db, g_dict["id"])
                    if plan:
        ...
        if choice == "0":
            ...
            if SESSION:
                SESSION.invalidate_after_goal_change()
            print(f"\n  Goal saved: {title}")
...
def run_decision_log():
    ...
    while True:
        ...
        # Show persona learning report if lessons exist
        if _LEARNING_ENGINE_AVAILABLE:
            try:
                learning_report = format_learning_report(db)
                if learning_report:
                    print()
                    divider("·")
                    for line in learning_report.split("\n"):
                        print(f"  {line}")
                    divider("·")
                    print()
            except Exception:
        ...
        if choice == "0":
            ...
            if pending:
                ...
                # Fire persona learning engine in background thread
                if _LEARNING_ENGINE_AVAILABLE:
                    try:
                        from core.groq_client import chat_completion
                        trigger_learning_from_rating(db, decision_id, chat_completion)
                    except Exception:
                        pass
                input("  Press ENTER to continue...")
            else:
...
def _run_static_profile_update():
    ...
    print()
    existing = dict(db.get_static_profile()) if db.get_static_profile() else {}
    profile  = {}
...
def _run_life_history_update():
    ...
    print()
    existing = dict(db.get_life_history()) if db.get_life_history() else {}
    history  = {}
...
def main():
    ...
    # Initialize session state — fetches all session-static data once
    if SESSION is not None:
        try:
            SESSION.initialize()
            log_info("SessionState", SESSION.summary())
        except Exception as e:
            log_warning("SessionState", f"Init failed: {str(e)[:60]}")
            SESSION = None

    run_startup_sequence()

    # Start background intelligence runner
    if _BACKGROUND_RUNNER_AVAILABLE:
        try:
            started = start_background_runner(db, logger_fn=log_info)
            if started:
                log_info("BackgroundRunner", "Daemon started — pre-warming context cache in 60s")
        except Exception:
            pass
