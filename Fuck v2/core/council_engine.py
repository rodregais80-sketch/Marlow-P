...
def _call_persona(persona, user_input, shared_context, history):
    """
    Thread-safe worker for council execution.
    Builds full persona-specific context and returns a structured response.
    """

    try:
        persona_name = persona.get("name", "UNKNOWN")

        # Tiered memory (if available)
        if _TIERED_MEMORY_AVAILABLE:
            tiered_ctx = _get_tiered_ctx(shared_context["db"], persona_name)
        else:
            tiered_ctx = ""

        # Persona lessons (if available)
        if _PERSONA_LEARNING_AVAILABLE:
            lessons_ctx = _get_lessons(shared_context["db"], persona_name, limit=5)
        else:
            lessons_ctx = ""

        # Pattern engine context
        pattern_ctx = shared_context.get("pattern_context", "")

        # Predictive crash window context
        predictive_ctx = shared_context.get("predictive_context", "")

        # Substance-outcome correlation context
        substance_ctx = shared_context.get("substance_context", "")

        # Decision quality context
        decision_ctx = shared_context.get("decision_context", "")

        # Context relevance flags
        if _CONTEXT_RELEVANCE_AVAILABLE:
            relevance_flags = _get_ctx_flags(shared_context["db"])
        else:
            relevance_flags = {}

        # Build final persona-specific context block
        full_context = (
            ...
            + "\n\n[Substances]\n" + substance_ctx
            + "\n\n[Decision Quality]\n" + decision_ctx
            + "\n\n[Relevance Flags]\n" + json.dumps(relevance_flags)
        )

        # Debate engine integration (if available)
        if _DEBATE_ENGINE_AVAILABLE:
            session_id = shared_context.get("session_id", generate_session_id())
            persona_reasoning_patterns = get_persona_reasoning_patterns(persona_name)
        else:
            session_id = "no_session"
            persona_reasoning_patterns = ""

        # Persona call (Groq or fallback)
        # NOTE: groq_chat (chat_completion) returns a plain string — not a dict.
        # model= is not a valid parameter for chat_completion; GROQ_MODEL is set
        # inside groq_client.py via module-level constant.
        if groq_available():
            persona_output = groq_chat(
                messages=[
            ...
            # Offline fallback — persona dict has no "fallback" key,
            # use the pre-written offline messages instead.
            import random as _random
            offline_pool = _OFFLINE_MESSAGES.get(persona_name, [f"{persona_name} is unavailable."])
            persona_output = _random.choice(offline_pool)

        # Debate engine storage
        if _DEBATE_ENGINE_AVAILABLE:
            store_council_reasoning(
                session_id=session_id,
                persona=persona_name,
                reasoning=persona_output,
                patterns=persona_reasoning_patterns
            )

        return {
            "persona": persona_name,
            "response": persona_output,
            "error": None
        }
...
def get_groq_key():
    ...
    # Root project dir (parent of core/)
...
def run_council(
    ...
    def _fire_persona(persona, messages, stagger_delay):
        """
        Inline worker for the pre-built messages path.
        build_persona_prompt already assembled the full messages list.
        Just stagger, call groq_chat, return structured result.
        """
        try:
            if stagger_delay > 0:
            ...
            persona_name = persona.get("name", "UNKNOWN")
            if groq_available():
                response = groq_chat(messages=messages)
            else:
                import random as _random
                offline_pool = _OFFLINE_MESSAGES.get(persona_name, [f"{persona_name} is unavailable."])
                response = _random.choice(offline_pool)
            return {"persona": persona_name, "response": response, "error": None}
        except Exception as e:
            return {"persona": persona.get("name", "UNKNOWN"), "response": None, "error": str(e)}

    round1_results = {}
    with ThreadPoolExecutor(max_workers=max(1, len(tasks))) as executor:
        future_map = {
            executor.submit(_fire_persona, task[0], task[1], task[3]): task[0]["name"]
            for task in tasks
        }
