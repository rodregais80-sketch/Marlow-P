...
class PredictiveEngine:
        ...
        try:
            ...
            cursor.execute("""
                ...
                (decision_text, category, energy_at_decision, mood_at_decision,
                 sleep_at_decision, fog_at_decision, impulse_at_decision, substance_context)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (decision_text, category, energy, mood, sleep, fog, impulse, substance_context))
            self.db.conn.commit()
    ...
    def rate_decision(self, decision_id: int, quality_rating: int, outcome_text: str = ""):
        ...
        try:
            self.db.conn.execute("""
                UPDATE predictor_decisions
                SET quality_rating = ?, outcome_text = ?, reviewed_at = ?
                WHERE id = ?
            """, (quality_rating, outcome_text, datetime.now(timezone.utc).isoformat(), decision_id))
            self.db.conn.commit()
        except Exception:
            pass
    ...
    def get_unrated_decisions(self, days_old: int = 30) -> list:
        ...
        try:
            cursor = self.db.conn.cursor()
            cursor.execute("""
                SELECT * FROM predictor_decisions
    ...
    def get_all_decisions(self, limit: int = 20) -> list:
        ...
        try:
            cursor = self.db.conn.cursor()
            cursor.execute(
                "SELECT * FROM predictor_decisions ORDER BY timestamp DESC LIMIT ?", (limit,)
            )
            return cursor.fetchall()
        except Exception:
            return []
                ...
                if days_since >= 3:
                    triggers.append({
                        ...
                        "content": (
                            ...
                            f"Last entry was {last_ts.strftime('%A %B %d')}. "
    ...
    def format_interventions_for_display(self) -> str:
        ...
        high     = [t for t in triggers if t["urgency"] == "HIGH"]
        moderate = [t for t in triggers if t["urgency"] == "MODERATE"]
