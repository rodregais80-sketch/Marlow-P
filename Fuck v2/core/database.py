...
class DatabaseManager:
    ...
    def _initialize_schema(self):
        ...
        # ── council_reasoning_log ─────────────────────────────────────────────
        # Stores per-persona arguments from every council round.
        # round_number 1 = initial analysis, round_number 2 = debate final position.
        # Used for longitudinal reasoning pattern analysis.
        # "ALDRIC warned about this exact pattern 3 months ago."
        self.conn.execute("""
        ...
        # ── context_cache ─────────────────────────────────────────────────────
        # Caches pre-computed heavy context blocks so they are not recomputed
        # on every persona call within the same session.
        # cache_key: identifies what was computed (e.g. "pattern_ctx", "momentum_ctx")
        # expires_at: ISO timestamp after which the cache entry is stale
        # session_tag: optional grouping for same-session entries
        self.conn.execute("""
        ...
        self.conn.execute("""
            ...
            ON context_cache (cache_key, expires_at)
        """)
        # Index for reasoning log persona lookups
        self.conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_reasoning_log_persona
            ON council_reasoning_log (persona, timestamp)
        """)
    ...
    # ── Context cache ─────────────────────────────────────────────────────────
    # Pre-computed heavy context blocks (pattern, prediction, momentum, streak,
    # contradiction) are cached here so they are not recomputed multiple times
    # within the same session or within a short window.
    # PatternEngine and PredictiveEngine are expensive reads — they should run
    # once per session, not once per persona call.

    def set_context_cache(self, cache_key: str, content: str,
                          ttl_minutes: int = 30, session_tag: str = "") -> None:
        """
        Stores a pre-computed context block in the cache.
        ttl_minutes: how long this entry is valid (default 30 min per session).
        """
        try:
            ...
            self.conn.execute(
                "INSERT INTO context_cache (cache_key, content, session_tag, expires_at) VALUES (?, ?, ?, ?)",
                (cache_key, content, session_tag, expires)
            )
            self.conn.commit()
        except Exception:
            pass

    def get_context_cache(self, cache_key: str) -> str:
        """
        Retrieves a cached context block if not expired.
        Returns empty string if not found or expired.
        """
        try:
            row = self.conn.execute(
                """
                SELECT content FROM context_cache
                WHERE cache_key = ?
    ...
    def clear_context_cache(self, session_tag: str = "") -> None:
        """
        Clears cache entries for a session tag and all expired entries.
        Prevents unbounded growth.
        """
        try:
            if session_tag:
                self.conn.execute(
                    "DELETE FROM context_cache WHERE session_tag = ?",
                    (session_tag,)
                )
            self.conn.execute(
                "DELETE FROM context_cache WHERE datetime(expires_at) <= datetime('now')"
            )
            self.conn.commit()
        except Exception:
            pass

    def get_recent_reasoning(self, persona_name: str = None, limit: int = 5) -> list:
        """
        Retrieves recent council reasoning log entries.
        Used in synthesis meta-context: what has the council argued recently?
        """
        try:
            if persona_name:
